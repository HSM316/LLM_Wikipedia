...
def load_error_and_null_ids(error_file, null_file):
    ...
    with open(error_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if "jpn-Jpan-nucl1643" in language:  # 只记录中文翻译错误的ID
    ...
    with open(null_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            language = parts[1].split(":")[1].strip()
            if "jpn-Jpan-nucl1643" in language:  # 只记录中文翻译空值的ID
...
def compute_bleu(reference, hypothesis):
    bleu = sacrebleu.corpus_bleu([hypothesis], [[reference]], tokenize='ja-mecab')
    return bleu.score
...
def process_and_compare(benchmark_file_1, benchmark_file_2, translated_file, output_csv, error_file, null_file):
    ...
    for translated_item in translated_data:
        ...
        translated_chinese = translated_item.get('translated_japanese', '')
        ...
        if id_ in error_ids or id_ in null_ids:
            for item in origin_benchmark:
                if item['id'] == id_:
                    origin_translation = item['translations'].get('jpn-Jpan-nucl1643(日语)', '')
            ...
            for item in gpt_benchmark:
                if item['id'] == id_:
                    gpt_translation = item['translations'].get('jpn-Jpan-nucl1643(日语)', '')
        ...
        bleu_scores.append({
            'id': id_,
            'translated_japanese': translated_chinese,
    ...
    with open(output_csv, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['id', 'translated_japanese', 'origin_translation', 'origin_bleu', 'gpt_translation', 'gpt_bleu']
...
translated_file = 'jap_translated_output.json'
output_csv = 'comet_scores_jap.csv'

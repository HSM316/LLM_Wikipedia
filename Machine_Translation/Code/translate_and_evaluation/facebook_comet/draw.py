...
def plot_bleu_scores(file_names, languages, language_names):
    ...
    for file_name in file_names:
        with open(file_name, 'r', encoding='utf-8') as f:
            reader = csv.reader(f)
            rows = list(reader)
            origin_bleu_avg = float(rows[-2][1])*100
            gpt_bleu_avg = float(rows[-1][1])*100
    ...
    plt.plot(sorted_languages, sorted_origin_bleu_scores, marker='o', label='Origin comet Avg', color='blue')
    plt.plot(sorted_languages, sorted_gpt_bleu_scores, marker='o', label='GPT comet Avg', color='green')

    plt.title('comet Score Comparison for Facebook/Nllb Models', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig('comet_scores_comparison_sorted.png')
...
language_names = ['Arabic', 'German', 'Spanish', 'French', 'Hindi', 'Italian', 'Japanese', 'Korean', 'Portuguese',
                  'Russian', 'Chinese']
file_names = [f"comet_scores_{lang}.csv" for lang in languages]

...
def load_error_and_null_ids(error_file, null_file):
    ...
    with open(error_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if "ita-Latn-ital1282" in language:  # 只记录中文翻译错误的ID
    ...
    with open(null_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            language = parts[1].split(":")[1].strip()
            if "ita-Latn-ital1282" in language:  # 只记录中文翻译空值的ID
...
def process_and_compare(benchmark_file_1, benchmark_file_2, translated_file, output_csv, error_file, null_file):
    ...
    for translated_item in translated_data:
        ...
        translated_chinese = translated_item.get('translated_italian', '')
        ...
        if id_ in error_ids or id_ in null_ids:
            for item in origin_benchmark:
                if item['id'] == id_:
                    origin_translation = item['translations'].get('ita-Latn-ital1282(意大利语)', '')
            ...
            for item in gpt_benchmark:
                if item['id'] == id_:
                    gpt_translation = item['translations'].get('ita-Latn-ital1282(意大利语)', '')
        ...
        bleu_scores.append({
            'id': id_,
            'translated_italian': translated_chinese,
    ...
    with open(output_csv, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['id', 'translated_italian', 'origin_translation', 'origin_bleu', 'gpt_translation', 'gpt_bleu']
...
translated_file = 'it_translated_output.json'
output_csv = 'comet_scores_it.csv'

...
def plot_bleu_scores(file_names, languages, language_names):
    ...
    plt.plot(sorted_languages, sorted_origin_bleu_scores, marker='o', label='Origin BLEU Avg', color='blue')
    plt.plot(sorted_languages, sorted_gpt_bleu_scores, marker='o', label='GPT BLEU Avg', color='green')

    plt.title('BLEU Score Comparison for Facebook/nllb Models', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig('bleu_scores_comparison_sorted.png')
...
language_names = ['Arabic', 'German', 'Spanish', 'French', 'Hindi', 'Italian', 'Japanese', 'Korean', 'Portuguese',
                  'Russian', 'Chinese']
file_names = [f"bleu_scores_{lang}.csv" for lang in languages]

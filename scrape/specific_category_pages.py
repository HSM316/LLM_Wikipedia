...
def get_pages_by_category(category_name, max_pages=10000):
    ...
    skip_pages = True  # 初始化为True，跳过所有页面
    saved_count = 0  # 计数器，记录已保存的页面数

    # 从 jsonl 文件加载已爬取的页面标题
    with open("D:/WIKIPEDIA/pages/Total_AI.jsonl", "r", encoding="utf-8") as infile:
        for line in infile:
            page = json.loads(line)
            pages.add(page["title"])

    with open("D:/WIKIPEDIA/pages/Total_AI_extra.jsonl", "w", encoding="utf-8") as file:
        while to_check_categories and saved_count < max_pages:
            ...
            for page in data.get("query", {}).get("categorymembers", []):
                ...
                if page["ns"] == 14:
                    ...
                    # 如果标题为“Subsumption lattice”，切换标志为False，从下一个页面开始处理
                    if page_title == "Acyl azide":
                        skip_pages = False
                        continue  # 跳过此页面，直接处理下一个页面

                    if skip_pages:
                        continue  # 在skip_pages为True时，跳过页面
